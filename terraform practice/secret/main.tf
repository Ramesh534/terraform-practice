resource "docker_service" "mysql_service" {
  ...
  task_spec {
    container_spec {
      ...
      secrets = [
        {
          file_name = "/run/secret/${docker_secret.mysql_root_password.name}"
          secret_id = "${docker_secret.mysql_root_password.id}"
          secret_name = "${docker_secret.mysql_root_password.name}"
        },
        {
          file_name = "/run/secret/${docker_secret.mysql_db_password.name}"
          secret_id = "${docker_secret.mysql_db_password.id}"
          secret_name = "${docker_secret.mysql_db_password.name}"
        }
      ]
      env {
        MYSQL_ROOT_PASSWORD_FILE = "/run/secret/${docker_secret.mysql_root_password.name}"
        MYSQL_DATABASE = "mydb"
        MYSQL_PASSWORD_FILE = "/run/secret/${docker_secret.mysql_db_password.name}"
      }
      mounts {
